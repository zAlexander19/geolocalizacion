...
export default function BuildingsPage() {
      ...
      render: (row) => {
        ...
        return (
          row.imagen && !/via\.placeholder\.com/.test(row.imagen) ? (
            <img
              src={row.imagen.startsWith('http')
                ? row.imagen
                : row.imagen.startsWith('/uploads')
                  ? `${window.location.origin}${row.imagen}`
                  : `http://localhost:4000${row.imagen}`}
              alt="Edificio"
              style={{ maxWidth: 48, maxHeight: 48, objectFit: 'cover', borderRadius: 8, cursor: 'pointer', background: '#e5e7eb' }}
              onClick={() => {
            ...
            <span className="text-xs text-gray-400 ml-2">Sin imagen</span>
          )
        )
      }
    },
                    ...
                    <td className="px-4 py-2">
                      {row.imagen && !/via\.placeholder\.com/.test(row.imagen) ? (
                        <img
                          ...
                          onError={e => { e.currentTarget.src = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48"><rect width="100%" height="100%" rx="12" fill="%23e5e7eb"/><rect x="12" y="12" width="24" height="24" rx="4" fill="%23222"/><rect x="18" y="18" width="6" height="6" fill="%23fff"/><rect x="24" y="18" width="6" height="6" fill="%23fff"/><rect x="18" y="24" width="6" height="6" fill="%23fff"/><rect x="24" y="24" width="6" height="6" fill="%23fff"/></svg>'; }}
                        />
                      ) : (
                        <img
                          src={'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48"><rect width="100%" height="100%" rx="12" fill="%23e5e7eb"/><rect x="12" y="12" width="24" height="24" rx="4" fill="%23222"/><rect x="18" y="18" width="6" height="6" fill="%23fff"/><rect x="24" y="18" width="6" height="6" fill="%23fff"/><rect x="18" y="24" width="6" height="6" fill="%23fff"/><rect x="24" y="24" width="6" height="6" fill="%23fff"/></svg>'}
                          alt="Sin imagen"
                          style={{ maxWidth: 48, maxHeight: 48, borderRadius: 8, background: '#e5e7eb' }}
                        />
                      )}
                    </td>
  ...
  <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Editar edificio' : 'Crear edificio'}>
        <Modal open={!!modalImg} onClose={() => setModalImg(null)} title="Imagen del edificio">
          {modalImg ? (
            <img
              src={modalImg}
              alt="Edificio"
              style={{ maxWidth: 400, maxHeight: 400, display: 'block', margin: '0 auto' }}
              onError={e => { e.currentTarget.style.display = 'none'; e.currentTarget.parentNode.append('No se pudo cargar la imagen'); }}
            />
          ) : (
        ...
        <Modal open={!!modalImg} onClose={() => setModalImg(null)} title="Imagen del edificio">
          {modalImg ? (
            <img src={modalImg} alt="Edificio" style={{ maxWidth: 400, maxHeight: 400, display: 'block', margin: '0 auto' }} />
          ) : (
            <span>No hay imagen</span>
          )}
        </Modal>
